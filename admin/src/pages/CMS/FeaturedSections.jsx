...
const FeaturedSections = () => {
  ...
  const fetchSections = async () => {
    try {
      const res = await api.get('/cms/featured-sections');
  ...
  const handleEditClick = (section) => {
    ...
    setShortDesc(section.shortDescription || '');
    setSeoDesc(section.seoKeywordDescription || '');
    ...
    showToast('Editing featured section...');
  };
  ...
  const handleCancelEdit = () => {
    ...
    setItems([{ title: '', name: '', image: '', categoryId: '', subCategoryId: '' }]);
    showToast('Featured section editing cancelled', 'warn');
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await api.put(`/cms/featured-sections/${editingId}`, payload);
        showToast('Featured section updated!');
      } else {
        await api.post('/cms/featured-sections', payload);
        showToast('Featured section created!');
      }

      handleCancelEdit();
      fetchSections();
    } catch {
      showToast('Failed to save featured section', 'error');
    }
  };
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      ...
      await api.put(`/cms/featured-sections/${id}`, { status: newStatus });
      fetchSections();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/cms/featured-sections/${id}`);
      fetchSections();
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Page Header */}
      <div>
        <h1 className="heading-1">Featured Sections</h1>
      </div>
                      ...
                      .map(p => {
                        ...
                        return (
                          <div
                            ...
                            className={`p-2 px-3 cursor-pointer rounded-md text-xs flex items-center justify-between mb-0.5 font-medium ${isSelected ? 'bg-rose-50 dark:bg-rose-950/30 text-rose-600 font-semibold' : 'bg-transparent text-admin-text-primary hover:bg-admin-hover'
                              }`}
                          >
                            <span>{p.name}</span>
